...
import { BarChart3, Dumbbell, Home, LogOut, Menu, NotebookTabs, Video } from 'lucide-react'
...
export default function PlayerLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen" style={{ background: brand.bg }}>
      ...
      <nav
        className="fixed inset-x-0 bottom-0 z-50 grid grid-cols-5 border-t px-1 py-2 md:hidden"
        style={{ background: brand.card, borderColor: brand.border }}
      >
        {navItems.map(({ href, label, icon: Icon, exact }) => {
          const active = isActive(href, exact)
